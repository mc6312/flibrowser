...
1. С http://python.org берется установщик Python 3.4.x (т.к. на момент написания
   этого README версии 3.5 и новее несовместимы с PyGI)
2. PyGI (с GTK 3.x в комплекте) ищется по ссылкам с гномьей вики -
   https://wiki.gnome.org/action/show/Projects/PyGObject?action=show&redirect=PyGObject#Binaries
